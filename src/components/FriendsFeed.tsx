      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 mb-4 px-1">
        ...
        {onOpenDbManager && (
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1 rounded-xl bg-sky-50 border border-sky-200 text-sky-700 hover:bg-sky-100 hover:border-sky-300 text-xs font-semibold transition-all shadow-2xs cursor-pointer"
          >
      ...
      <div className="bg-white border border-slate-200/90 rounded-2xl p-4 sm:p-5 shadow-2xs mb-6">
        <div className="flex flex-col md:flex-row items-center gap-4">
          ...
          <div className="relative w-full md:flex-1">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-sky-500" />
            <input
              ...
              className="w-full bg-slate-50 border border-slate-200 rounded-full pl-11 pr-4 py-2.5 text-xs sm:text-sm text-slate-800 placeholder-slate-400 focus:bg-white focus:outline-none focus:border-sky-500 focus:ring-2 focus:ring-sky-100 transition-all"
            />
            {searchQuery && (
              <button
          ...
          <div className="flex items-center gap-3 w-full md:w-auto justify-between md:justify-end px-2">
            <div className="flex items-center gap-2 text-xs text-slate-600">
              ...
              <strong className="text-sky-600 font-bold">{minMatch}%</strong>
            </div>
            <input
              type="range"
              min="50"
        ...
        <div className="flex flex-wrap items-center gap-2 mt-4 pt-4 border-t border-slate-100">
          <span className="text-xs font-semibold text-slate-400 mr-1">Filtro rápido:</span>
          ...
          >
            <Laugh className="w-3 h-3 text-amber-500" />
            Memes
          </button>
          ...
          >
            <CalendarCheck className="w-3 h-3 text-fuchsia-600" />
            Eventos
          </button>
          ...
          {activeTraitFilter && (
            <div className="ml-auto flex items-center gap-2 bg-sky-50 border border-sky-200 text-sky-700 px-3 py-1 rounded-full text-xs font-bold">
              <span>Rasgo: {activeTraitFilter}</span>
              <button onClick={onClearTraitFilter} className="hover:text-sky-900 cursor-pointer">✕</button>
            </div>
          )}
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {filteredFriends.map((friend) => (
            <div
              ...
              className="bg-white border border-slate-200/90 hover:border-sky-300 rounded-2xl overflow-hidden shadow-2xs transition-all duration-300 hover:shadow-md flex flex-col justify-between group"
            >
              <div>
                ...
                <div className="relative p-5 pb-0 flex items-center gap-3.5">
                  <div className="relative">
                    <img
                      ...
                      className="w-16 h-16 rounded-2xl object-cover border border-slate-200 group-hover:border-sky-300 transition-colors shadow-2xs"
                    />
                    <span 
                      className={`absolute -bottom-1 -right-1 px-1.5 py-0.5 rounded-full text-white text-[9px] font-bold shadow-xs ${
                        friend.dynamicCompatibility.score >= 90
                          ? 'bg-emerald-500'
                          : friend.dynamicCompatibility.score >= 75
                          ? 'bg-sky-500'
                          : 'bg-indigo-500'
                      }`}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-1">
                      <h3 className="text-base font-bold text-slate-800 truncate group-hover:text-sky-600 transition-colors">
                        {friend.name}, <span className="font-normal text-slate-400 text-xs">{friend.age}</span>
                      </h3>
                      <span className="text-[10px] font-mono text-slate-400 bg-slate-100 px-1.5 py-0.5 rounded">
                        id: {friend.id}
                      </span>
                    </div>
                    <p className="text-xs text-slate-500 truncate">{friend.occupation}</p>
                    <p className="text-[11px] text-sky-600 font-medium flex items-center gap-1 mt-0.5">
                      <MapPin className="w-3 h-3" /> {friend.city}
                    </p>
                    {friend.dynamicCompatibility.reasons.length > 0 && (
                      <div className="mt-1 flex items-center gap-1 text-[10px] text-emerald-700 bg-emerald-50/80 px-2 py-0.5 rounded-md border border-emerald-100 truncate">
                ...
                <div className="px-5 py-3">
                  ...
                  <div className="flex flex-wrap gap-1.5 mt-3">
                    {friend.traits.slice(0, 3).map((trait) => (
                      <span
                        key={trait}
                        className="px-2.5 py-0.5 rounded-md text-[10px] font-medium bg-slate-100 text-slate-600 border border-slate-200/60"
                      >
                        #{trait}
                      </span>
                    ))}
                    {friend.traits.length > 3 && (
                      <span className="px-1.5 py-0.5 rounded-md text-[10px] text-sky-700 font-medium bg-sky-50 border border-sky-100">
                        +{friend.traits.length - 3}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="mt-3.5 pt-3 border-t border-slate-100 space-y-1.5 text-[11px] bg-slate-50/70 p-2.5 rounded-xl border border-slate-100">
                    <div className="flex items-center justify-between gap-1.5 text-slate-600">
                      <div className="flex items-center gap-2 truncate flex-1 min-w-0">
                        ...
                        <span className="text-slate-400 font-medium">Cocina:</span>
                        <span className="truncate text-slate-800 font-bold">{friend.favoriteFood}</span>
                      </div>
                      {isAdmin && onEditFriendInDb && (
                        <button
                          type="button"
                          onClick={(e) => { e.stopPropagation(); onEditFriendInDb(friend); }}
                          className="p-1 rounded-md text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 transition-colors cursor-pointer shrink-0"
                    ...
                    <div className="flex items-center gap-2 text-slate-600 truncate">
                      <Laugh className="w-3.5 h-3.5 text-amber-500 flex-shrink-0" />
                      <span className="text-slate-400 font-medium">Humor:</span>
                      <span className="truncate text-slate-700 font-medium">{friend.favoriteMemeStyle}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="p-4 pt-3 border-t border-slate-100 bg-white flex items-center gap-2">
                ...
                >
                  {friend.isConnected ? <UserCheck className="w-3.5 h-3.5 text-emerald-600" /> : <UserPlus className="w-3.5 h-3.5" />}
                  <span>{friend.isConnected ? 'Conectado' : 'Conectar'}</span>
                </button>

                <button
                  onClick={() => onOpenFriendDetail(friend)}
                  className="py-2 px-3 rounded-xl text-xs font-semibold bg-slate-100 hover:bg-slate-200 text-slate-700 border border-slate-200/80 flex items-center justify-center gap-1.5 transition-colors cursor-pointer"
                >
                ...
                {isAdmin && onEditFriendInDb && (
                  <button
                    ...
                    className="py-2 px-2.5 rounded-xl text-xs font-bold bg-indigo-50 hover:bg-indigo-100 text-indigo-700 border border-indigo-200 flex items-center justify-center gap-1 transition-colors cursor-pointer"
                  >
